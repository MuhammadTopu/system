...
async registerUserStep1(email) {
  ...
  const existingTempUser = await this.userRepository.findTempUserByEmail(email);
...
async verifyOTP(otp, email) {
  ...
  const verifiedUser =
    await this.userRepository.createVerifiedUser(
      notVerifiedUser.email
    );

  await this.userRepository.deleteTempUser(
    notVerifiedUser.id
  );
...
async registerUserStep3({ token, name, password }) {
  ...
  const existingUser =
    await this.userRepository.findByEmail(decoded.email);
  ...
  const updatedUser =
    await this.userRepository.completeRegistration({
      email: decoded.email,
      name,
...
async forgotPasswordOTPsend(email) {
  ...
  const existingTempUser =
    await this.userRepository.findTempByEmail(email);
...
async changePassword({ userId, body }) {
  ...
  const user =
    await this.userRepository.findByIdAndType(userId, "USER");
...
async sendMailToAdmin({ user, body }) {
  ...
  const userData =
    await this.userRepository.findUserByEmailAndId({
      email: user_email,
      id: userId,
      type: "USER",
    });
